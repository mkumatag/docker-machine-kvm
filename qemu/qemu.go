...
const (
	...
	domainXMLTemplate = `<domain type='{{.DomainType}}'>
  ...
  <devices>
    <disk type='file' device='cdrom'>
      <source file='{{.ISO}}'/>
      <target dev='{{.CdDeviceName}}' bus='{{.CdBusType}}'/>
      <readonly/>
    </disk>
    <disk type='file' device='disk'>
      <driver name='qemu' type='raw' cache='{{.CacheMode}}' io='{{.IOMode}}' />
      <source file='{{.DiskPath}}'/>
      <target dev='{{.DiskDeviceName}}' bus='{{.DiskBusType}}'/>
    </disk>
...
func (d *Driver) getConn() (*libvirt.VirConnection, error) {
	if d.conn == nil {
		...
		if err != nil {
			...
			return &libvirt.VirConnection{}, errors.New("Unable to connect to qemu driver, did you add yourself to the libvirtd group?")
		}
		d.conn = &conn
	}
	return d.conn, nil
...
func (d *Driver) setArchConfigs() error {
	log.Debug("setArchConfigs called")
	conn, err := d.getConn()
	if err != nil {
		return err
	}
	...
	if err != nil {
		log.Errorf("Unable to get the Capabilities %s", err)
		return err
	}
	...
	c := Capabilities{}
	err = xml.Unmarshal([]byte(capabilities), &c)
	if err != nil {
		log.Errorf("Unable to decode the Capabilities %s", err)
		return err
	}

	found := false
	for _, guest := range c.Guest {
		if guest.Arch.Name == d.Arch {
			found = true
			d.DomainType = "qemu"
			for _, domain := range guest.Arch.Domain {
				if domain.Type == "kvm" {
					d.DomainType = domain.Type
				}
			}
		}
	}
	...
	if !found {
		return fmt.Errorf("Unable to locate the emulator for '%s' in capabilities, please make sure you have a proper emulator installed", d.Arch)
	}

	if d.Arch == "ppc64" || d.Arch == "ppc64le" {
		d.CdDeviceName = "sda"
		d.CdBusType = "scsi"
		d.DiskDeviceName = "vda"
		d.DiskBusType = "virtio"
		d.CPUtype = `<cpu mode='custom' match='exact'>
					<model fallback='allow'>POWER8</model>
					</cpu>`
	} else {
		d.CdDeviceName = "hdc"
		d.CdBusType = "ide"
		d.DiskDeviceName = "hda"
		d.DiskBusType = "ide"
		d.CPUtype = `<cpu mode='custom' match='exact'>
					</cpu>`
	}
...
func (d *Driver) getMAC() (string, error) {
	...
	*/
	...
	log.Debugf("interface dom object: %#v", dom.Devices)
